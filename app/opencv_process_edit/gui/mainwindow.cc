  ...
  : FancyWindow(parent)
{
  ...
  connect(timer_, SIGNAL(timeout()), this, SLOT(timeupFun()));
...
void MainWindow::createQuickAccessBar()
{
  if (FancyQuickAccessBar* quickAccessBar = fancyBar()->quickAccessBar())
  {
    ...
    new_action_ = quickAccessBar->addAction(QIcon(":/tools/new"), tr("New"));
    new_action_->setShortcut(QKeySequence::New);
    new_action_->setToolTip(tr("New File or Project\nCtrl+N"));
    connect(new_action_, SIGNAL(triggered()), this, SLOT(slotNew()));

    open_action_ = quickAccessBar->addAction(QIcon(":/tools/open"), tr("Open"));
    open_action_->setShortcut(tr("Ctrl+O"));
    open_action_->setToolTip(tr("Open File or Project\nCtrl+O"));

    QMenu* menu = new QMenu(this);
    QAction* open_action_file = menu->addAction(tr("file"));
    connect(open_action_file, SIGNAL(triggered()), this, SLOT(slotOpen()));

    menu->addAction(tr("action2"));
    menu->addAction(tr("action3"));
    menu->addAction(tr("action4"));
    open_action_->setMenu(menu);

    save_action_ = quickAccessBar->addAction(QIcon(":/tools/save"), tr("Save"));
    save_action_->setShortcut(tr("Ctrl+S"));
    save_action_->setToolTip(tr("Save the active document\nCtrl+S"));
    connect(save_action_, SIGNAL(triggered()), this, SLOT(slotSave()));

    save_all_action_ = quickAccessBar->addAction(QIcon(":/tools/save_all"), tr("Save All"));
    save_all_action_->setShortcut(tr("Ctrl+Shift+S"));
    save_all_action_->setToolTip(tr("Save all active documents\nCtrl+Shift+S"));

    undo_action_ = quickAccessBar->addAction(QIcon(":/tools/undo"), tr("&Undo"));
    undo_action_->setShortcut(QKeySequence::Undo);
    undo_action_->setEnabled(false);

    redo_action_ = quickAccessBar->addAction(QIcon(":/tools/redo"), tr("&Redo"));
    redo_action_->setShortcut(QKeySequence::Redo);
    redo_action_->setEnabled(false);
    quickAccessBar->setActionVisible(redo_action_, false);
...
void MainWindow::createModeBar()
{
  ...
  m_pScenerMode = new ScenerMode();
  m_pMenuMode = new MenuMode();
  m_pProcessEditMode = new ProcessEditMode();

  m_modeManager->addMode(m_pCustomMode);
  m_modeManager->addMode(m_pChartsMode);
  m_modeManager->addMode(m_pScenerMode);
  m_modeManager->addMode(m_pMenuMode);
  m_modeManager->addMode(m_pProcessEditMode);

  m_modeManager->setCurrentMode(m_pProcessEditMode);
...
{
  QString string;
  m_modeManager->currentMode()->save(string); 
  /* 
  QDialog dialog;
  FancyBanner banner;
  for (int i = 0; i < 10; i++) {
...
{
  QString str;
  m_modeManager->currentMode()->open(str);
}
