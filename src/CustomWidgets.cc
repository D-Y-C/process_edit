...
eParamTrackEdit::eParamTrackEdit(eF32 &flt, Parameter &param, QWidget *parent)
    : eTrackEdit(flt, param.min, param.max, parent), m_indEnabled(false)
{
...
eParamTrackEdit::eParamTrackEdit(eInt &intgr, Parameter &param, QWidget *parent)
    : eTrackEdit(intgr, param.min, param.max, parent), m_indEnabled(false)
{
  connect(this, SIGNAL(onValueChanged()), this, SLOT(_onValueChanged()));
}

eParamTrackEdit::eParamTrackEdit(eU8 &byte, Parameter &param, QWidget *parent)
    : eTrackEdit(byte, param.min, param.max, parent), m_indEnabled(false)
{
  connect(this, SIGNAL(onValueChanged()), this, SLOT(_onValueChanged()));
}

void eParamTrackEdit::_onValueChanged() { Q_EMIT onParameterChanged(); }
...
void eParamTrackEdit::paintEvent(QPaintEvent *pe)
{
  ...
  if (m_indEnabled) {
    ...
    if (brightness < 123) textColor = Qt::white;
...
// push button
//------------------------------------------------------------------------------------------------------------------------------
eButton::eButton(Parameter &param, QWidget *parent) : QPushButton(parent), m_param(param)
{
...
void eButton::_onClicked() { Q_EMIT onParameterChanged(m_param); }

//------------------------------------------------------------------------------------------------------------------------------
// boolean button
//------------------------------------------------------------------------------------------------------------------------------

eBoolButton::eBoolButton(Parameter &param, QWidget *parent) : QToolButton(parent), m_param(param)
{
...
void eBoolButton::_updateCaption()
{
  ...
  setText(std::get<bool>(m_param.getBaseValue()) ? "Yes" : "No");
}

void eBoolButton::_onClicked()
{
  // m_param.getBaseValue().get<bool>() = !m_param.getBaseValue().get<bool>();
  std::get<bool>(m_param.getBaseValue()) = !std::get<bool>(m_param.getBaseValue());
  _updateCaption();
  m_param.setChanged();
...
eComboBox::eComboBox(Parameter &param, QWidget *parent) : QComboBox(parent), m_param(param)
{
  ...
  for (eInt i = 0; i < descrItems.size(); i++) addItem(descrItems[i]);
...
eFlagButton::eFlagButton(Parameter &param, const QString &caption, eU32 flagIndex, QWidget *parent)
    : QToolButton(parent), m_param(param), m_flagIndex(flagIndex)
{
...
eTextEdit::eTextEdit(Parameter &param, QWidget *parent) : m_param(param)
{
...
eLineEdit::eLineEdit(Parameter &param, QWidget *parent) : m_param(param)
{
...
void eLineEdit::_updateCaption() { setText(m_param.getValueAsString()); }
...
eFileFrame::eFileFrame(Parameter &param, Type type = Type::SAVE, QWidget *parent)
    : QWidget(parent), m_param(param), _type(type)
{
...
void eFileFrame::_updateCaption() { m_lineEdit->setText(m_param.getValueAsString()); }
...
void eFileFrame::_onSelectLocally()
{
  ...
  if (_type == Type::OPEN) {
    filename = QFileDialog::getOpenFileName(
        nullptr, tr("Load File"), QDir::currentPath(), tr("all files (*.*)"));
...
eColorFrame::eColorFrame(Parameter &param, QWidget *parent) : QWidget(parent), m_param(param)
{
  ...
  for (eU32 i = 0; i < param.getComponentCount(); i++) {
    ...
    eU8 &var = std::get<eColor>(m_param.getBaseValue())[i];
    m_edits[i] = new eParamTrackEdit(var, param);
    hbl->addWidget(m_edits[i]);
    // connect(m_edits[i], SIGNAL(onParameterChanged(const Parameter &)), this,
    // SLOT(_updateEditColors())); connect(m_edits[i], SIGNAL(onParameterChanged(const Parameter
    // &)), this, SIGNAL(onParameterChanged(const Parameter &)));
    connect(
        m_edits[i], &eParamTrackEdit::onParameterChanged, this, &eColorFrame::_updateEditColors);
    connect(
        m_edits[i], &eParamTrackEdit::onParameterChanged, this, &eColorFrame::onParameterChanged);
...
void eColorFrame::_onSelectLocally()
{
  ...
  eColor &pc = std::get<eColor>(m_param.getBaseValue());
