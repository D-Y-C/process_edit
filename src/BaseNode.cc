...
unsigned int BaseNode::getParameterCount() { return (unsigned int)_parameters.size(); }
...
NodeValidationState BaseNode::validationState() const
{
  ...
  if (_parameters.size() > 0 && _paramWidget == nullptr) {
    ...
    // qWarning("Warning: _paramWidget was not created for node '%s'",
    // Name().toStdString().c_str());
...
QString BaseNode::validationMessage() const { return modelValidationError; }
