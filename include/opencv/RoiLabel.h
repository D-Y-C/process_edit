...
public:
  ...
  explicit RoiLabel(const QString &text, QWidget *parent = nullptr, Qt::WindowFlags f = Qt::WindowFlags());
	~RoiLabel();
	QRect getRoiRect() const;               //获取已经圈选的框 外部调用
	void setBackImage(const QImage &img);   //设置背景图片  外部调用
  void getRoiImage(QImage &img) const;
...
Q_OBJECT
public:
	RoiDialog(QWidget *parent = nullptr);
	~RoiDialog();

	void SetImage(const QImage &img);

signals: 
	void SelectRoiImag(const QImage &img);

private slots:
  void SureBtnSlot(void);
	void CancelBtnSlot(void);
